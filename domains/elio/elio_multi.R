...
plot_elio_prop2_data <- function(auto, free, f_pch) {
  # This function assume the global workspace variables used at the time it is called below
  
  f_graphname <- paste(ifelse(free, "sc_", ""), "l",ifelse(PLOT_LC, "c", ""),ifelse(PLOT_L1,"1",""),"2",ifelse(auto, "3",""), "_t", T, sep="")
  f_inpath <- paste("/home/bryan/Documents/Research/PRIMsDuplications/Elio/",ifelse(PLOT_VERBOSE,"verbose_",""),"elio_props_", f_graphname,ifelse(PLOT_VERBOSE,"_X1pr",""),".dat", sep="")
  f_data <- read.table(f_inpath)
  f_data <- data.frame(f_data[1:13], f_data[11]+f_data[13])
  names(f_data) <- c("task","trial","line","RT","answer", "DC1s", "chunks", "sample", "DCs", "ST1", "ST2", "fails", "latencies", "ST")
  
  f_data$ST <- f_data$DCs*0.05
...
plot_elio_prop3_data <- function(auto, cog, f_pch) {
  # This function assume the global workspace variables used at the time it is called below
  
  f_graphname <- paste("l",ifelse(PLOT_L1,"1",""),ifelse(PLOT_L2,"2",""),ifelse(auto, "3",""), "_t", T, samples, sep="")
  f_inpath <- paste("/home/bryan/Documents/GitHub_Bryan-Stearns/PROPs/domains/elio/results/",ifelse(PLOT_VERBOSE,"verbose_",""),"elio_props_", f_graphname,ifelse(PLOT_VERBOSE,"_X",""),".dat", sep="")
  f_data <- read.table(f_inpath)
  names(f_data) <- c("task","trial","line","ST","answer", "DCs", "sample")
...
PLOT_EPSETS <- FALSE       # The case where fetch-guiding sets are used (true) or not (false)
...
yfloor <- 0
legendY <- 0.99
savename = paste("fig_elio_prop3_",ifelse(PLOT_ACTR, "m",""),ifelse(PLOT_PROPv1, "1",""),ifelse(PLOT_PROPv2, "2",""),ifelse(PLOT_PROPv240, "240",""),ifelse(PLOT_PROPdev, "3",""),ifelse(PLOT_SPREADING,"_","_"),sep="")
...
maintxt <- ifelse(PLOT_ACTR,"Actransfer KNOWN-AUTO", paste("",ifelse(PLOT_PROPv240,"KNOWN-",""),ifelse(PLOT_PROPv2, "Arithmetic - PROP2", "Arithmetic - PROP3"),sep=""))
legendText <- c("Human")
legendPch <- c(4)
...
if (PLOT_PROPv2) {legendText <- c(legendText,ifelse(PLOT_PROPv240 && !PLOT_SPREADING,"KNOWN 50ms","PROP2")); legendPch<-c(legendPch, 1)}
...
if (PLOT_PROPdev) {
  #plot_elio_prop2_data(PLOT_L3, TRUE, 2); legendText <- c(legendText, paste("LEARNED",sep="")); legendPch <- c(legendPch, 2);
  #plot_elio_prop2_data(PLOT_L3, FALSE, 2); legendText <- c(legendText, paste("PROP2 No retrievals",ifelse(PLOT_EPSETS," proposals",""),sep="")); legendPch <- c(legendPch, 2);
  plot_elio_prop3_data(PLOT_L3, FALSE, 2); legendText <- c(legendText, paste("PROP3",ifelse(PLOT_EPSETS," proposals",""),sep="")); legendPch <- c(legendPch, 1);
}

title(xlab="Trial", ylab=ylabel, line=2)

legend(ifelse(PLOT_SPREADING,3,13),legendY*yscale,legend=legendText,lty=c(1,1,1,1,1),pch=legendPch, col=c("gray37","black","black","black","black"), pt.cex=1, cex=1.0)
