...
plot_editor_prop2_data <- function(auto, free, f_pch) {
  ...
  f_inpath <- paste("/home/bryan/Documents/Research/PRIMsDuplications/Elio/verbose_test_elio_props_", f_graphname,"_X1pr",".dat", sep="")
...
graphname = paste("l", ifelse(PLOT_L3 && !PLOT_L2 && !PLOT_L1, 
                              ...
                              paste(ifelse(PLOT_LC, "c", ""), ifelse(PLOT_L1, "1", ""), ifelse(PLOT_L2, "2", ""), ifelse(PLOT_L3, "3", ""), sep="")), 
                  "_t", T, sep="")
...
if (PLOT_PROPv2 && PLOT_PROPv240) {legendText <- c(legendText,ifelse(PLOT_PROPv240 && !PLOT_SPREADING,"KNOWN 50ms","AUTO")); legendPch<-c(legendPch, 1)}
...
# PLOT
png(paste("/home/bryan/Dropbox/UM_misc/Soar/Research/PROPs/PRIMs_Duplications/Elio/results/", savename, prefix, PLOT_TASK, "_", graphname, ".png", sep=""),
...
if (PLOT_PROPv2 && PLOT_PROPv240) {lines((1:10*5),prop2_data.m[1,,taskInd],type="b",pch=1,lty=1,ylim=c(yfloor,yscale),xlim=c(0,xscale),main=title,col="black")}
...
if (!PLOT_ACTR && !PLOT_PROPv240) {
  plot_editor_prop2_data(PLOT_L3, TRUE, 2); legendText <- c(legendText, paste("LEARNED",sep="")); legendPch <- c(legendPch, 2);
  #plot_editor_prop2_data(PLOT_L3, FALSE, 1); legendText <- c(legendText, paste("KNOWN",sep="")); legendPch <- c(legendPch, 1);
}
