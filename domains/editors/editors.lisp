...
(add-instr ed :input (Vtype Vword1 Vword2 Vline) :working-memory (WMcurline WMsearch-goal) 
  ...
  ;~~ enter
  (ins :condition (Gcontrol = line-strategy  RT1 = success) :action (find-line -> Gcontrol) :description "No, so we press enter") ;;; Less than three: use enter strategy
  ;~~ pcmd
  ...
  ;~~ re-next
  (ins :condition (WMsearch-goal <> RTsecond RTfirst = WMcurline Gcontrol = find-line) :action (RTsecond -> WMcurline enter -> AC1 (count-fact RTsecond nil) -> RT) :description "Not done yet, pressing enter again")
  ;~~ re-read
