...
plot_wmspan <- function(ylims, dat1, dat2, leg) {
  ...
  if (!missing(dat2)) {
    ...
    legend(1,ylims[2],legend=leg,pch=c(2,1),lty=1,bty="n")
  }
}
...
graphname <- ifelse(PLOT_SOURCE=="PRIMS", "", 
                   paste("_", ifelse(PLOT_L1, "1", ""), ifelse(PLOT_L2, "2", ""), ifelse(PLOT_L3, "3", ""), 
                         "s", "_t", t, sample, sep=""))
...
vdat <- read.table(paste(modelpath,"WMChein",graphname,".txt",sep=""))
names(vdat) <- c("day","span","correct")
vdat.m <- with(vdat[vdat$correct==1,],tapply(span,day,mean))
#vdat.m <- with(vdat,tapply(span,day,mean))

# Non-rehearsal model VCWM data:
vdatNR <- read.table(paste(modelpath,"WMCheinNR",graphname,".txt",sep=""))
names(vdatNR) <- c("day","span","correct")
vdatNR.m <- with(vdatNR[vdatNR$correct==1,],tapply(span,day,mean))
#vdat.m <- with(vdat,tapply(span,day,mean))

# Rehearsal model Stroop data:
sdat <- read.table(paste(modelpath,"stroopChein",graphname,".txt",sep=""))
...
sdatNR <- read.table(paste(modelpath,"stroopCheinNR",graphname,".txt",sep=""))
names(sdatNR) <- c("task","condition","block","day","trial","type","correct","RT")
